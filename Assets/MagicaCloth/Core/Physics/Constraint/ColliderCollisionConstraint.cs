...
namespace MagicaCloth
{
    ...
    public class ColliderCollisionConstraint : PhysicsManagerConstraint
    {
        ...
        struct CollisionJob : IJobParallelFor
        {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    if (colliderMap.TryGetFirstValue(colliderTeam, out cindex, out iterator))
                    {
                        do
                        {
                            ...
                            {
                                // カプセルコライダー判定
                                hit = CapsuleColliderDetection(ref nextpos, basepos, radius, cindex, new float3(1, 0, 0), keep);
                            }
                            else if (cflag.IsFlag(PhysicsManagerParticleData.Flag_CapsuleY))
                            {
                                // カプセルコライダー判定
                                hit = CapsuleColliderDetection(ref nextpos, basepos, radius, cindex, new float3(0, 1, 0), keep);
                            }
                            else if (cflag.IsFlag(PhysicsManagerParticleData.Flag_CapsuleZ))
                            {
                                // カプセルコライダー判定
                                hit = CapsuleColliderDetection(ref nextpos, basepos, radius, cindex, new float3(0, 0, 1), keep);
                            }
                            ...
                            {
                                // 球コライダー判定
                                hit = SphereColliderDetection(ref nextpos, basepos, radius, cindex, keep);
                            }
                            ...
                            if (hit)
                            {
                                // 衝突あり！
                                // フラグに設定
                                //flag.SetFlag(PhysicsManagerParticleData.Flag_Friction, true);
                                // 摩擦設定
            ...
            /// <returns></returns>
            bool SphereColliderDetection(ref float3 nextpos, float3 basepos, float radius, int cindex, bool keep)
            {
                ...
                // n = 平面方向
                // 平面衝突判定と押し出し
                bool ret = MathUtility.IntersectPointPlane(c, n, nextpos, out nextpos);
            ...
            /// <param name="friction"></param>
            /// <returns></returns>
            bool CapsuleColliderDetection(ref float3 nextpos, float3 basepos, float radius, int cindex, float3 dir, bool keep)
            {
                // lpos.x = 長さ（片側）
                // lpos.y = 始点半径
                // lpos.z = 終点半径
            ...
            /// <param name="radius"></param>
            /// <param name="cindex"></param>
            bool PlaneColliderDetection(ref float3 nextpos, float radius, int cindex)
            {
                // 平面姿勢
                var cpos = nextPosList[cindex];
                var crot = nextRotList[cindex];
                ...
                // n = 平面方向
                // 平面衝突判定と押し出し
                bool ret = MathUtility.IntersectPointPlane(cpos, n, nextpos, out nextpos);
