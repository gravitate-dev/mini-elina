...
namespace MagicaCloth
{
    ...
    public class PhysicsManagerMeshData : PhysicsManagerAccess
    {
        ...
        public const uint Meshflag_CalcNormalTangent = 0x00000002;
        ...
        /// <summary>
        /// 仮想メッシュインスタンスが接続しているレンダーメッシュ情報
        /// ※これはインスタンスごとのデータ
        /// x = レンダーメッシュインスタンスインデックス
        /// y = レンダーメッシュの頂点スタートインデックス
        /// z = レンダー共有メッシュの頂点スタートインデックス
        /// </summary>
        //public ExNativeMultiHashMap<int, int3> virtualToRenderMeshIndexMap;
        ...
        public struct SharedRenderMeshInfo
        {
            ...
            // 各成分のデータ有無
            //public bool existNormals;
            //public bool existTangents;
            //public bool existWeights;
        ...
        /// レンダーメッシュ頂点フラグ
        /// </summary>
        public const uint RenderVertexFlag_Use = 0x00010000; // 使用フラグ
        ...
        /// <summary>
        /// レンダーメッシュインスタンスが接続している仮想メッシュ情報
        /// ※これはレンダーメッシュインスタンスごとのデータ
        /// x = 子共有メッシュの頂点スタートインデックス
        /// y = 子共有メッシュのウエイトスタートインデック
        /// z = 仮想メッシュの頂点スタートインデックス
        /// w = 仮想共有メッシュの頂点スタートインデックス
        /// </summary>
        ...
        public override void Create()
        {
            ...
            //virtualToRenderMeshIndexMap = new ExNativeMultiHashMap<int, int3>();
            ...
            renderToChildMeshIndexMap = new ExNativeMultiHashMap<int, int4>();
        ...
        public override void Dispose()
        {
            ...
            //virtualToRenderMeshIndexMap.Dispose();
            ...
            renderToChildMeshIndexMap.Dispose();
        ...
        public int VirtualMeshVertexUseCount
        {
            get
            {
                // 少々重いので注意
                int cnt = 0;
                for (int i = 0; i < virtualVertexInfoList.Length; i++)
                    if ((virtualVertexInfoList[i] & 0xffff) > 0)
                        cnt++;
                return cnt;
            }
        }
            ...
            )
        {
            ...
            minfo.sharedRenderMeshVertexStartIndex = sharedRenderMeshInfoList[sharedMeshIndex].vertexChunk.startIndex;
        ...
        public void RemoveRenderMesh(int renderMeshIndex)
        {
            ...
            if (renderMeshInfoList.Exists(renderMeshIndex))
            {
                ...
                // レンダーメッシュとマージメッシュのリンクをすべて削除
                renderToChildMeshIndexMap.Remove(renderMeshIndex);
        ...
        {
            return renderMeshInfoList[renderMeshIndex].IsUse();
        }
        ...
        public void LinkRenderMesh(int renderMeshIndex, int childMeshVertexStart, int childMeshWeightStart, int virtualMeshVertexStart, int sharedVirtualMeshVertexStart)
        {
            ...
            renderToChildMeshIndexMap.Add(renderMeshIndex, new int4(childMeshVertexStart, childMeshWeightStart, virtualMeshVertexStart, sharedVirtualMeshVertexStart));
        }
        ...
        public void UnlinkRenderMesh(int renderMeshIndex, int childMeshVertexStart, int childMeshWeightStart, int virtualMeshVertexStart, int sharedVirtualMeshVertexStart)
        {
            ...
            renderToChildMeshIndexMap.Remove(renderMeshIndex, new int4(childMeshVertexStart, childMeshWeightStart, virtualMeshVertexStart, sharedVirtualMeshVertexStart));
        }
        ...
        {
            var minfo = renderMeshInfoList[renderMeshIndex];
            var sminfo = sharedRenderMeshInfoList[minfo.renderSharedMeshIndex];
            var buffer = sharedRenderMeshIdToBufferDict[sminfo.uid];
            renderPosList.ToJobArray().CopyToFast(minfo.vertexChunk.startIndex, buffer.vertices);
        ...
        public void CopyToRenderMeshLocalNormalTangentData(int renderMeshIndex, Mesh mesh)
        {
            ...
            var sminfo = sharedRenderMeshInfoList[minfo.renderSharedMeshIndex];
            var buffer = sharedRenderMeshIdToBufferDict[sminfo.uid];
            //if (sminfo.existNormals)
            if (sminfo.IsFlag(MeshFlag_ExistNormals))
            {
                renderNormalList.ToJobArray().CopyToFast(minfo.vertexChunk.startIndex, buffer.normals);
                mesh.normals = buffer.normals;
            }
            //if (sminfo.existTangents)
            if (sminfo.IsFlag(MeshFlag_ExistTangents))
            {
                renderTangentList.ToJobArray().CopyToFast(minfo.vertexChunk.startIndex, buffer.tangents);
...
#if UNITY_2018
            renderBoneWeightList.ToJobArray().CopyToFast(minfo.vertexChunk.startIndex, buffer.boneWeights);
            mesh.boneWeights = buffer.boneWeights;
#else
            NativeArray<BoneWeight1> weights = new NativeArray<BoneWeight1>(minfo.boneWeightsChunk.dataLength, Allocator.Temp);
            renderBoneWeightList.ToJobArray().CopyToFast(minfo.boneWeightsChunk.startIndex, weights);
        ...
        public int RenderMeshUseCount
        {
            get
            {
                int cnt = 0;
                for (int i = 0; i < renderMeshInfoList.Length; i++)
                    cnt += renderMeshInfoList[i].IsUse() ? 1 : 0;
                return cnt;
            }
        }
        ...
        public int RenderMeshVertexUseCount
        {
            get
            {
                // 重いので注意
                int cnt = 0;
                for (int i = 0; i < renderVertexFlagList.Length; i++)
                    if ((renderVertexFlagList[i] & RenderVertexFlag_Use) != 0)
                        cnt++;
                return cnt;
            }
        }
