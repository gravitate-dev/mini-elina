...
namespace MagicaCloth
{
    ...
    public class PhysicsManagerParticleData : PhysicsManagerAccess
    {
        ...
        //public const uint Flag_Transform_Restore_Pos = 0x00040000; // 実行前にトランスフォームlocalPosを復元
        //public const uint Flag_Transform_Restore_Rot = 0x00080000; // 実行前にトランスフォームlocalRotを復元
        ...
        public override void Create()
        {
            ...
            //var c = CreateParticle(Flag_Kinematic, 0, 0, quaternion.identity, 0, quaternion.identity, 0.0f, 1.0f, null, 0);
            ...
            )
        {
            ...
            //{
            //    // 復元トランスフォーム
            //    if (pf.IsRestoreTransform())
            //    {
            //        restoreTransformIndex = Bone.AddRestoreBone(target, lpos, lrot);
            //    }

            //    // 読み込みトランスフォーム
            //    if (pf.IsReadTransform())
            //    {
            //        // パーティクル書き戻し判定
            //        int windex = pf.IsWriteTransform() ? c.startIndex : -1;

            //        transformIndex = Bone.AddBone(target, windex);
            //    }
            //}
            ...
            )
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                //        {
                //            // パーティクル書き戻し判定
                //            int windex = pf.IsWriteTransform() ? pindex : -1;
        ...
        public void RemoveParticle(ChunkData c)
        {
            for (int i = 0; i < c.dataLength; i++)
            {
                ...
                //    if (pf.IsRestoreTransform())
                //    {
                //        var restoreTransformIndex = restoreTransformIndexList[pindex];
                ...
                //    if (pf.IsReadTransform())
                //    {
                //        var transformIndex = transformIndexList[pindex];
                //        int windex = pf.IsWriteTransform() ? pindex : -1;
                    ...
                    if (funcTarget != null)
                    {
                        ...
                        if (target != null)
                        {
                            ...
                            if (flag.IsRestoreTransform() && restoreTransformIndexList[index] == -1)
                            {
                                ...
                                //byte rflag = 0;
                                //rflag |= flag.IsFlag(Flag_Transform_Restore_Pos) ? PhysicsManagerBoneData.RestoreBone_Flag_RestoreLocalPos : (byte)0;
                                //rflag |= flag.IsFlag(Flag_Transform_Restore_Rot) ? PhysicsManagerBoneData.RestoreBone_Flag_RestoreLocalRot : (byte)0;

                                //restoreTransformIndexList[index] = Bone.AddRestoreBone(target, lpos, lrot, rflag);
                            ...
                            if (flag.IsReadTransform() && transformIndexList[index] == -1)
                            {
                                ...
                                transformIndexList[index] = Bone.AddBone(target, windex);
                            }

                            //restoreTransformIndexList[index] = restoreTransformIndex;
