    ...
    public class VirtualMeshWorker : PhysicsManagerWorker
    {
        ...
        private struct CalcMeshTriangleNormalTangentJob : IJobParallelFor
        {
            ...
            public void Execute(int tindex)
            {
                ...
                // 計算フラグを立てる
                virtualVertexFlagList[vindex0] = 1;
                virtualVertexFlagList[vindex1] = 1;
                virtualVertexFlagList[vindex2] = 1;
        ...
        private struct CalcVertexNormalTangentFromTriangleJob : IJobParallelFor
        {
            ...
            {
                // 計算フラグ頂点のみ
                if (virtualVertexFlagList[vindex] == 0)
                    return;
...
#if false
        [BurstCompile]
        private struct CalcMeshTriangleVertexNormalTangentJob : IJobParallelFor
        {
            [ReadOnly]
            public NativeArray<PhysicsManagerMeshData.VirtualMeshInfo> virtualMeshInfoList;
            [ReadOnly]
            ...
            {
                // このメッシュの使用をチェック
                var m_minfo = virtualMeshInfoList[mindex];
                if (m_minfo.IsUse() == false)
                    return;
                int m_vstart = m_minfo.vertexChunk.startIndex;
                int m_tstart = m_minfo.triangleChunk.startIndex;
                ...
                // 頂点フラグクリア
                int vcnt = m_minfo.vertexChunk.dataLength;
                for (int i = 0; i < vcnt; i++)
                {
                    int vindex = m_vstart + i;
                    virtualVertexFlagList[vindex] = 0;
                }

                // トライアングルグループ
                int tcnt = m_minfo.triangleChunk.dataLength;
                for (int i = 0; i < tcnt; i++)
                {
                    int tindex = m_tstart + i;
                ...
                // 頂点ループ
                int s_vtstart = s_minfo.vertexToTriangleChunk.startIndex;
                for (int i = 0; i < vcnt; i++)
                {
                    int vindex = m_vstart + i;
                    ...
                    uint pack = sharedVirtualVertexToTriangleInfoList[s_vstart + i];
                    int sv_tcnt = MeshUtility.Unpack8_24Hi(pack);
                    int sv_tstart = MeshUtility.Unpack8_24Low(pack);
                    if (sv_tcnt == 0)
                        continue;
                    sv_tstart += s_vtstart;
                    ...
                    float3 nor = 0;
                    float3 tan = 0;
                    for (int j = 0; j < sv_tcnt; j++)
                    {
                        int tindex = sharedVirtualVertexToTriangleIndexList[sv_tstart + j] + m_tstart;
                        nor += virtualTriangleNormalList[tindex];
                        tan += virtualTriangleTangentList[tindex];
                    }
                    nor = math.normalize(nor);
