...
namespace MagicaCloth
{
    ...
    public static class EditorInspectorUtility
    {
        ...
        public static bool AdjustLineInspector(SerializedProperty cparam)
        {
            EditorGUI.BeginChangeCheck();

            var avarage = cparam.FindPropertyRelative("useLineAvarageRotation");

            const string title = "Adjust Line";
            Foldout(title, title,
                () =>
                {
                    avarage.boolValue = EditorGUILayout.Toggle("Avarage Rotation", avarage.boolValue);
                }
                );
